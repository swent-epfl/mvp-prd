...
First, we have the maps, allowing the user to see the precise location of his posts and current challenges. Moreover, they are also able to display all the posts that are less than one hundred meter away.
Additionally, the user is also able to access the heatmap, enabling them to see places around the world that are currently attracting a lot of interaction.
